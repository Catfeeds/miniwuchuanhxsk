      ...
      <table class="pro_3">
         ...
         <!-- 遍历 -->
          <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "暂时没有数据" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
             <td><?php echo ($v["id"]); ?></td>
             <td style="padding:3px 0;"><img src="/miniwuchuanhxsk/Data/<?php echo ($v["photo"]); ?>" width="80px" height="80px"/></td>
             <!-- <td><?php echo ($v["shangchang"]); ?></td> -->
             <td><?php echo ($v["name"]); ?></td>
             <td><?php echo ($v["price"]); ?></td>
             <td><?php if($v["is_recomed"] == 1): ?><label style="color:green;">推荐</label><?php endif; ?></td>
            <td>
              <a href="<?php echo U('set_tj');?>?pro_id=<?php echo ($v["id"]); ?>&page=<?php echo ($page); ?>&shop_id=<?php echo ($shop_id); ?>">推荐</a> |
              <a href="<?php echo U('Hotel/add');?>?id=<?php echo ($v["id"]); ?>&page=<?php echo ($page); ?>&shop_id=<?php echo ($shop_id); ?>">修改</a> |
